...
NAV = """
      ...
      <a href="../descargas/KK-5S-User-Manual-EN.pdf">Download PDF</a>
...
INDEX = """
      ...
      <h2>Downloads</h2>
      <p>Offline copies. The PDF is the full manual with HMI screenshots.</p>
      <div class="cards">
        <a class="card" href="descargas/KK-5S-Manual-Operacion-ES.pdf" download>
          <div class="num">PDF</div>
          <h3>Spanish manual</h3>
          <p>Single file for print, phone, or shop-floor USB.</p>
        </a>
        <a class="card" href="descargas/KK-5S-User-Manual-EN.pdf" download>
          <div class="num">PDF</div>
          <h3>English manual</h3>
          <p>Full English operator manual, same figures and tables.</p>
        </a>
        <a class="card" href="descargas/KK-5S-manual-offline.zip" download>
          <div class="num">ZIP</div>
          <h3>Offline website</h3>
          <p>HTML + images. Open <code>index.html</code> in any browser.</p>
        </a>
        <a class="card" href="original/Cold_Cutting_Flying_Saw_Instruction_Manual.docx" download>
          <div class="num">DOCX</div>
          <h3>Original Word file</h3>
          <p>Factory English manual, May 2019 V1.36.</p>
        </a>
      </div>
...
def main():
    ...
    es_list = pages_es(es_fig, table, param, "assets/img/")
    for name, title, crumb, body in es_list:
        html = page(
    ...
    for name, title, crumb, body in PAGES:
        body = (
            body.replace("assets/img/", "../assets/img/")
            .replace('href="original/', 'href="../original/')
            .replace('href="descargas/', 'href="../descargas/')
        )
        html = page(
            name, title, crumb, body,
            lang="en",
            ...
            nav=NAV,
            es_href=pair.get(name, "../index.html"),
            en_href=name,
        )
    ...
    write_print_all(
        OUT / "print-all.html",
        lang="es",
        title="Manual completo KK-5S",
        chapters=es_list,
        asset_prefix="assets/",
        img_already_prefixed=True,
        nav=nav_es(),
        brand_sub="Sierra volante de corte en frío",
        skip="Saltar al contenido",
        menu="Abrir menú",
        search_ph="Buscar en el manual…",
        foot="Manual KK-5S · Mayo 2019 V1.36",
        meta="KK-5S · Manual V1.36",
        es_href="print-all.html",
        en_href="en/print-all.html",
    )
    en_chapters = [
        (name, title, crumb, body.replace("assets/img/", "../assets/img/"))
        for name, title, crumb, body in PAGES
    ]
    write_print_all(
        en_dir / "print-all.html",
        lang="en",
        title="KK-5S complete manual",
        ...
        menu="Open menu",
        search_ph="Search the manual…",
        foot="KK-5S user manual · May 2019 V1.36",
        meta="KK-5S · User manual V1.36",
        es_href="../print-all.html",
        en_href="print-all.html",
    )


def write_print_all(path, *, lang, title, chapters, asset_prefix, img_already_prefixed, **page_kw):
    parts = []
    for _name, _title, _crumb, body in chapters:
        parts.append(f'<section class="print-chapter">\n{body}\n</section>')
    html = page(
        path.name,
        title,
        "Print",
